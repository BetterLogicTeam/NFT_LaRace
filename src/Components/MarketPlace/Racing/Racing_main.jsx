...
export default function Racing_main() {
  ...
  const showIdDetail = async (tokenid) => {
    ...
    try {
      ...
      const itemNo = await contractOf.methods
        .itemNo(wireNftContractAddress, tokenid)
        .call();
  ...
  const startRace = async (tokenid) => {
    ...
    try {
      ...
      if (userInfo.activate) {
        if (
          parseFloat(web3.utils.fromWei(userInfo.Getreward)) <
          parseFloat(web3.utils.fromWei(userInfo.pkgNotoID))
        ) {
          if (parseInt(userInfo.perDay) <= 12) {
            let createBid = await contractOf.methods
          ...
          toast.info(
            `${tokenid} reward is reached to ${web3.utils.fromWei(
              userInfo.pkgNotoID
            )}, so boos your id`
          );
          setSpinnerLoading(false);
                      ...
                      <Link to="#">
                        <li class="active">
                          <div class="item-tab">
                            ...
                            <a href="Racing.html" class="item-tab-title">
                              Live Racing
                            </a>
                          </div>
                        </li>
                      ...
                      <Link to="/Items/Finished_race">
                        <li class="">
                          <div class="item-tab">
                            ...
                            <a
                              href="Racing_Finished.html"
                              class="item-tab-title"
                            >
                              Races History
                            </a>
                          </div>
                      ...
                      <div class="viewTable">
                        <div class="viewPega ">
                          <h5 className="text-white">
                            Please Select Horse For Race
                          </h5>

                          <div class="list-pick">
                            {Array_NFT.map((items, idex) => {
                              let index = idex + 1;
                              //  ()=> showIdDetail(items.Tokenid)
                              return (
                                <>
                                  <div class="item-pega">
                                    <a
                                      href="#"
                                        onClick={() => showIdDetail(items.Tokenid)}
                                    >
                                      <div class="item-box">
